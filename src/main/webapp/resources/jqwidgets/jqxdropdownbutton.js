	...
	a.extend(a.jqx._jqxDropDownButton.prototype, {
		...
		render: function () {
			...
			if (b.autoOpen) {
				...
				f.addHandler(a(document), "mousemove." + b.element.id, function (j) {
					var i = b.isOpened();
					if (i && b.autoOpen) {
						var n = b.host.coord();
						var m = n.top;
						var l = n.left;
						var k = b.container.coord();
						var g = k.left;
						var h = k.top;
						canClose = true;
						if (j.pageY >= m && j.pageY <= m + b.host.height()) {
							if (j.pageX >= l && j.pageX < l + b.host.width()) {
								canClose = false
							}
						}
						if (j.pageY >= h && j.pageY <= h + b.container.height()) {
							if (j.pageX >= g && j.pageX < g + b.container.width()) {
								canClose = false
							}
						}
						if (canClose) {
		...
		destroy: function () {
			...
			b.host.removeClass();
			b.removeHandler(a(document), "mousedown." + b.element.id, self.closeOpenedDropDown);
			b.host.remove();
			b.container.remove()
		},
		_raiseEvent: function (g, c) {
			...
			if (g == 2 && !f.contentInitialized) {
				if (f.initContent) {
					f.initContent();
					f.contentInitialized = true
				}
			}
			var d = f.events[g];
			args = c;
